...
double xs_vs_p_09(double *x, double *par)
{
  const double deg=atan(1.0)/45.;
  double Ef=x[0];
  ...
  double Theta=par[3]*deg;
  double xs=0.0;
  if(E0>Ef) xs = PBosted::GetXS(Z,N,E0,Ef,Theta,0,0,false);
  //if(isnan(xs)) xs=0.0;
...
void plotxs()
{
  ...
  TF1 *f4 = new TF1("f4",xs_vs_p_09,0.8,2.2,4);
  f4->SetParameters(2,1,2.100,11.7);
  f4->SetLineColor(4);
  f4->Draw("same");
}
